...
export default function ConsultaTipoCliente({ navigation }) {
  ...
  const data = [
    { key: '1', value: 'Público' },
    { key: '2', value: 'Mayoreo' },
    { key: '3', value: 'Medio Mayoreo' },
    { key: '4', value: 'Distribución' },
  ];
  ...
  const leeRegistros = async () => {
    const respuestaPOST = await fetch('http://'+ip+':3000/ConsultaTipoCliente', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({TipoCliente: selected})
      });
    ...
    <View>
      ...
      <DataTable>
        ...
        <DataTable.Header style={styles.header} textStyle={styles.textheader}>
          <DataTable.Title textStyle={styles.textheader}>id Cliente</DataTable.Title>
        ...
        {records.map((reg) =>
            (
              <DataTable.Row key={reg.idCliente}>
                <DataTable.Cell>{reg.idCliente}</DataTable.Cell>
