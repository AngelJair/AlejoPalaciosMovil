...
export default function PagosXFormaPago({ navigation }) {
  ...
  const leeRegistros = async () => {
    const respuestaPOST = await fetch('http://'+ip+':3000/ConsultaPagosXForma', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({Forma: selected})
      });
  ...
  return (
    ...
    <View>
      ...
      <DataTable.Header style={styles.header} textStyle={styles.textheader}>
          <DataTable.Title textStyle={styles.textheader}>id Pago</DataTable.Title>
          <DataTable.Title textStyle={styles.textheader}>Proveedor</DataTable.Title>
        ...
        {records.map((reg) =>
            (
              <DataTable.Row key={reg.idPago}>
                <DataTable.Cell>{reg.idPago}</DataTable.Cell>
