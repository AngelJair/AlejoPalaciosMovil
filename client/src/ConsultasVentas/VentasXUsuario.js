...
export default function VentasXUsuario({ navigation }) {
  ...
  const [Usuario, addUsuario] = React.useState([]);

  const [records, addRecords] = React.useState([]);


  const leeRegistros = async () => {
    const respuestaPOST = await fetch('http://'+ip+':3000/ConsultaVentasXUsu', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({Usu: selected})
      });
  ...
  React.useEffect(() => {
    ...
    const SubirCtes = async () => {
      fetch('http://'+ip+':3000/SubirUsuarios', {
        method: 'GET',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
  ...
  return (
    ...
    <View>
      <SelectList
        ...
        data={Usuario.map((reg) =>(reg.Nombre))}
        save="value"
        placeholder='Usuario'
      ...
      <DataTable>
        ...
        <DataTable.Header style={styles.header} textStyle={styles.textheader}>
          ...
          <DataTable.Title textStyle={styles.textheader} >Usuario</DataTable.Title>
        ...
        {records.map((reg) =>
            (
              <DataTable.Row key={reg.idVenta}>
                <DataTable.Cell>{reg.idVenta}</DataTable.Cell>
