...
export default function ComprasVencidas({ navigation }) {
  ...
  const leeRegistros = async () => {
    const respuestaPOST = await fetch('http://'+ip+':3000/ConsultaComprasVencidas', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify(
          {
            FV: dateI.toLocaleDateString("zh-Hans-CN"), 
          },
          )
      });
        ...
        <View style={styles.btnContainer}>
          {!isPickerShowI && (
            <View style={styles.btn} icon={<MaterialIcons name="date-range"/>}>
              <Button title="Fecha Vencimiento" color='#4147BF' onPress={showPickerI} />
            </View>
          )}
