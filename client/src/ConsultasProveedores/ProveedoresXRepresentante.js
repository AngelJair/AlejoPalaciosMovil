...
export default function ProveedoresXRepresentante({ navigation }) {
  ...
  const [Representante, addRepresentante] = React.useState([]);
  ...
  const leeRegistros = async () => {
    const respuestaPOST = await fetch('http://'+ip+':3000/ConsultaProvRepre', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({Repre: selected})
      });
  ...
  React.useEffect(() => {
    ...
    const SubirRepres = async () => {
      fetch('http://'+ip+':3000/SubirRepres', {
        method: 'GET',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
        ...
        .then( (markers_response) => {
          addRepresentante(markers_response);
        } )
        .catch(error => {
          console.error(error);
      });
  ...
  return (
    ...
    <View>
      <SelectList
        ...
        data={Representante.map((reg) =>(reg.Representante))}
        save="value"
